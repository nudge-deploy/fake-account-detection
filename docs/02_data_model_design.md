...
- **login_sessions**: Riwayat aktivitas login yang merekam IP, geo-location, dan penggunaan VPN/Proxy. Relasi `N:1` ke `users` dan `devices`.
...
Tabel `fake_account_abt` adalah hasil agregasi (Feature Engineering) dari semua tabel di atas. Setiap baris mewakili satu `user_id` dan berisi metrik-metrik yang siap dimasukkan ke dalam model Machine Learning:
...
- **Login Features:** Kecepatan login beruntun (login velocity), dll.
- **Graph/Network Features:** `graph_degree`, ukuran kluster jaringan (seberapa terhubung user tersebut ke user lain melalui device/address/payment).
- **Output/Label:** `risk_score_rule_based`, `risk_category`, `is_fake_account`.
