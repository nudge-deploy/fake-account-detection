/*
Purpose: Proxy frontend transaction-completed inference requests to the backend lifecycle endpoint.
Used by: Inference page in the Next.js app when simulating transaction completion.
...
export async function POST(request: Request) {
  ...
  const response = await fetch(`${BACKEND_URL}/api/inference/lifecycle`, {
    ...
    body: JSON.stringify({ ...body, stage: 'transaction_completed' }),
  });
